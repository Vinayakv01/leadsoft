...
interface Lead {
  ...
  keywords?: string;
  stage?: string;
  notes?: Array<{ id: number; text: string; timestamp: string; type: string }>;
  tasks?: Array<{ id: number; text: string; timestamp: string; completed: boolean; type: string }>;
...
interface LeadDetailModalProps {
  ...
  onAddNote: (note: { id: number; text: string; timestamp: string; type: string }) => void;
  onAddTask: (task: { id: number; text: string; timestamp: string; completed: boolean; type: string }) => void;
}

function LeadDetailModal({ lead, onClose, onUpdate, onAddNote, onAddTask }: LeadDetailModalProps) {
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-280px)]">
          ...
          {activeTab === 'activities' && (
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                {lead.activities?.length ? lead.activities.map((activity, i) => (
          ...
          {activeTab === 'tasks' && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {lead.tasks?.length ? lead.tasks.map((task, i) => (
          ...
          {activeTab === 'notes' && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {lead.notes?.length ? [...lead.notes].reverse().map((note, i) => (
...
export default function Home() {
  ...
  const handleAddNote = (note: { id: number; text: string; timestamp: string; type: string }) => {
    if (selectedLead) {
      const updatedLead = {
        ...selectedLead,
  ...
  const handleAddTask = (task: { id: number; text: string; timestamp: string; completed: boolean; type: string }) => {
    if (selectedLead) {
      const updatedLead = {
        ...selectedLead,
  ...
  const stats = useMemo(() => ({
    total: leads.length,
    byStage: leads.reduce((acc: Record<string, number>, lead) => {
  ...
  const stageConfig: Record<string, { label: string; color: string; bg: string; text: string }> = {
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                ...
                {(crmData.industries || []).slice(0, 20).map((ind: string) => (
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-100">
                  {paginatedLeads.map((lead) => {
                    const stage = stageConfig[lead.stage || 'new'] || stageConfig.new;
                    return (
